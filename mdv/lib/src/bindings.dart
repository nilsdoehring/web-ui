 ...
 * registered with MDV via [TemplateExtension.syntax]:
 *
 *
 * HTML:
 ...
 *     ...
 *
 *     TemplateExtension.syntax['MySyntax'] = new MySyntax();
 *
...
// TODO(jmesserly): should this move into dart:observe?
class CompoundBinding extends ObservableMixin {
  CompoundBindingCombinator _combinator;

  // TODO(jmesserly): ideally these would be String keys, but sometimes we
  // use integers.
  Map<dynamic, _Binding> _bindings = new Map();
  Map _values = new Map();
  ...
  void set value(newValue) {
    _value = notifyChange('value', _value, newValue);
  }

  // TODO(jmesserly): remove getValue/setValue when dart2js supports mirrors!
  getValue(key) {
  ...
  void bind(name, model, String path) {
    ...
    _bindings[name] = new _Binding(model, path, (value) {
      _values[name] = value;
      _scheduleResolve();
    });
  }
...
// TODO(jmesserly): we can probably inline this object as it doesn't do much.
class _Binding {
  final PathObserver _path;
  StreamSubscription _sub;

  _Binding(model, String path, _ChangeHandler changed)
      : _path = observePath(model, path) {
    if (_path == null) {
      // TODO(jmesserly): Should we display an error message about invalid path?
      changed(null);
      return;
    }
    _sub = _path.values.listen(changed);
    changed(_path.value);
  }
  ...
  void set value(newValue) {
    if (_path != null) _path.value = newValue;
  }
}
...
abstract class _InputBinding {
  ...
  _InputBinding(this.element, model, String path) {
    binding = new _Binding(model, path, valueChanged);
    _sub = _getStreamForInputType(element).listen(updateBinding);
  }
  ...
  void updateBinding(e) {
    ...
    if (element is InputElement && element.type == 'radio') {
      for (var r in _getAssociatedRadioButtons(element)) {
        var checkedBinding = mdv(r)._checkedBinding;
...
Node _createDeepCloneAndDecorateTemplates(Node node, String syntax) {
  ...
  if (_isTemplate(clone)) {
    TemplateExtension.decorate(clone, node);
    if (syntax != null) {
...
HtmlDocument _getTemplateContentsOwner(HtmlDocument doc) {
  if (doc.window == null) {
    return doc;
  }
  var d = mdv(doc)._templateContentsOwner;
  if (d == null) {
    ...
    mdv(doc)._templateContentsOwner = d;
  }
  return d;
}
...
void _liftNonNativeTemplateChildrenIntoContent(Element templateElement) {
  var content = mdv(templateElement).content;

  if (!_isAttributeTemplate(templateElement)) {
    var child;
    while ((child = mdv(templateElement).$dom_firstChild) != null) {
...
void _bootstrapTemplatesRecursivelyFrom(Node node) {
  void bootstrap(template) {
    if (!TemplateExtension.decorate(template)) {
...
void _parseAndBind(Node node, String text, String name, model,
  ...
  mdv(node).bind(name, replacementBinding, 'value');
}
...
void _addTemplateInstanceRecord(fragment, model) {
  ...
  while (node != null) {
    mdv(node)._templateInstance = instanceRecord;
    node = node.nextNode;
  }
}
...
void _removeTemplateChild(Node parent, Node child) {
  mdv(child)._templateInstance = null;
  if (_isTemplate(child)) {
    // Make sure we stop observing when we remove an element.
    var templateIterator = mdv(child)._templateIterator;
    if (templateIterator != null) {
      templateIterator.abandon();
      mdv(child)._templateIterator = null;
    }
  }
  child.remove();
...
class _InstanceCursor {
  ...
  void next() {
    ...
    while (_index > mdv(_terminator)._instanceTerminatorCount) {
      _index -= mdv(_terminator)._instanceTerminatorCount;
  ...
  void abandon() {
    ...
    assert(mdv(_terminator)._instanceTerminatorCount > 0);
    assert(_index > 0);

    mdv(_terminator)._instanceTerminatorCount--;
    _index--;
  }
  ...
  void insert(fragment) {
    ...
    if (_terminator != _previousTerminator) {
      while (mdv(_previousTerminator)._instanceTerminatorCount >
              _previousIndex) {
        mdv(_previousTerminator)._instanceTerminatorCount--;
        mdv(_terminator)._instanceTerminatorCount++;
      }
    }
  }
...
class _TemplateIterator {
  ...
  _TemplateIterator(this._templateElement) {
    ...
    valueBinding = new _Binding(inputs, 'value', valueChanged);
  }

  Object resolveInputs(Map values) {
  ...
  void valueChanged(value) {
    ...
    if (value is Observable) {
      _sub = value.changes.listen(handleChange);
    }

    handleSplices([new ListChangeDelta(0, addedCount: iteratedValue.length)]);
  }

  // TODO(jmesserly): these properties appear not be finished. I think it's
  // part of custom syntax like repeat="i in items".
  getInstanceModel(model, syntax) => model;
  getInstanceFragment(syntax) => mdv(_templateElement).createInstance();

  void handleChange(List<ChangeRecord> records) {
    handleSplices(summarizeListChanges(iteratedValue, records));
  }

  void handleSplices(List<ListChangeDelta> splices) {
    var syntax = TemplateExtension.syntax[_templateElement.attributes['syntax']];

    for (var splice in splices) {
      for (var removed in splice.removed) {
...
int _instanceCount(Element element) {
  var templateIterator = mdv(element)._templateIterator;
